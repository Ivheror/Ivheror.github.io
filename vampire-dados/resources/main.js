...
function dice_initialize(container) {
    ...
    $t.bind(regularPool, 'input', function(ev) {
      const word = (ev.target.value == '1') ? 'Dado normal' : 'Dados normales';
      regularInfo.innerHTML = ev.target.value + ' ' + word;
    });
    $t.bind(bloodPool, 'input', function(ev) {
      const word = (ev.target.value == '1') ? 'Ansia' : 'Ansias';
    ...
    function after_roll(notation, result) {
      ...
      let newHtml = '';
      if (successes > 0) {
        newHtml += `${successes} éxito`;
        if (successes > 1) { newHtml += 's'; }
        if (messyCritical) {
          newHtml += ', <span class="messy-critical critical">Critico guapo</span>'
        } else if (critical) {
          newHtml += ', <span class="critical">Critico</span>'
        }
      } else {
        if (besitalFailure) {
          newHtml += '<span class="bestial-failure failure">Fallo bestial</span>';
        } else {
          newHtml += '<span class="failure">Fallo</span>';
        }
      }
